...
function parseColorFile(colorText: string): ColorMapping {
...
function asciiColor(colorCode: number, text: string): string {
...
async function imageToAsciiColor(
  ...
  for (let y = 0; y < info.height; y++) {
    for (let x = 0; x < info.width; x++) {
      ...
      const pixel: Pixel = [data[idx], data[idx + 1], data[idx + 2]];
